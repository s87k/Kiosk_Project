...
public class CouponInfoViewDAO {
	...
	public List<CouponAddedInfoVO> searchAllAddedCouponView() throws SQLException {
		...
		try {
			...
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String selectCoupKind = "select COUP_KIND_NO, COUP_KIND_NAME, DISCOUNT, FLAG_PUBLISHABLE, EXPIRES_PERIOD, CONDITION from COUPON_ADDED_INFO_VIEW";
			...
			while(rs.next()) {				// 쿼리문 실행했을 때 조회 결과가 있다면
				listCaivVO.add(new CouponAddedInfoVO(rs.getInt("COUP_KIND_NO"), rs.getString("COUP_KIND_NAME"), rs.getInt("discount"), rs.getString("flag_publishable").equals("0") ? false:true, rs.getInt("EXPIRES_PERIOD"), rs.getString("CONDITION")));
			} // end if

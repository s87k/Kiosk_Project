...
public class CouponKindDAO {
	...
	public List<CouponKindVO> selectAllCoupKind() throws SQLException {
		...
		try {
			...
			// 		값이 들어가는 위치는 바인드 변수 `?`를 사용한다
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String selectCoupKind = "select COUP_KIND_NO, ID, COUP_KIND_NAME, DISCOUNT, expires_period, INPUT_DATE, FLAG_PUBLISHABLE, DELETE_DATE, FLAG_DELETE from coupon_kind where flag_delete='0'";
			pstmt = con.prepareStatement(selectCoupKind);

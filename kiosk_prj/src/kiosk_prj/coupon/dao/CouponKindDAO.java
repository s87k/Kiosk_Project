...
public class CouponKindDAO {
	...
	public void insertCoupKind(CouponKindVO ckVO) throws SQLException {
		...
		try {
			...
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			StringBuilder insertCoupKind = new StringBuilder("insert into coupon_kind(COUP_KIND_NO, ID, COUP_KIND_NAME, DISCOUNT, expires_period, FLAG_PUBLISHABLE, DELETE_DATE, FLAG_DELETE, img) ")
	...
	public int updateCoupKind(CouponKindVO ckVO) throws SQLException {
		...
		try {
			...
			// 		값이 들어가는 위치는 바인드 변수 `?`를 사용한다
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String updateCoupKind = "update coupon_kind set coup_kind_name=?, FLAG_PUBLISHABLE=?, img=?  where coup_kind_no=?";
			...
			pstmt.setString(3, ckVO.getImg());
			pstmt.setInt(4, ckVO.getCoupKindNo());
	...
	public CouponKindVO selectOneCoupKind(int coupKindNo) throws SQLException {
		...
		try {
			...
			// 		값이 들어가는 위치는 바인드 변수 `?`를 사용한다
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String selectCoupKind = "select ID, COUP_KIND_NAME, DISCOUNT, expires_period, INPUT_DATE, FLAG_PUBLISHABLE, DELETE_DATE, FLAG_DELETE, img from coupon_kind where coup_kind_no=? and flag_delete='0'";
			...
			if(rs.next()) {				// 쿼리문 실행했을 때 조회 결과가 있다면
				ckVO = new CouponKindVO(coupKindNo, rs.getString("id"), rs.getString("coup_kind_name"), rs.getInt("discount"), rs.getInt("expires_period"), rs.getDate("input_date"), rs.getString("flag_publishable").equals("0") ? false:true, rs.getDate("delete_date"), rs.getString("flag_delete").equals("0") ? false : true, rs.getString("img"));
			} // end if
	...
	public List<CouponKindVO> selectAllCoupKind() throws SQLException {
		...
		try {
			...
			// 		값이 들어가는 위치는 바인드 변수 `?`를 사용한다
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String selectCoupKind = "select COUP_KIND_NO, ID, COUP_KIND_NAME, DISCOUNT, expires_period, INPUT_DATE, FLAG_PUBLISHABLE, DELETE_DATE, FLAG_DELETE, img from coupon_kind where flag_delete='0' order by FLAG_PUBLISHABLE desc, COUP_KIND_NO";
			pstmt = con.prepareStatement(selectCoupKind);
			...
			while(rs.next()) {				// 쿼리문 실행했을 때 조회 결과가 있다면
				list.add(new CouponKindVO(rs.getInt("coup_kind_no"), rs.getString("id"), rs.getString("coup_kind_name"), rs.getInt("discount"), rs.getInt("expires_period"), rs.getDate("input_date"), rs.getString("flag_publishable").equals("0") ? false:true, rs.getDate("delete_date"), rs.getString("flag_delete").equals("0") ? false : true, rs.getString("img")));
			} // end if
	...
	public List<CouponKindVO> selectFlagPublishCoupKind(boolean flagPublishable) throws SQLException {
		...
		try {
			...
			// 		값이 들어가는 위치는 바인드 변수 `?`를 사용한다
			// 		바인드 변수에는 `'` 를 사용하지 않는다
			String selectCoupKind = "select COUP_KIND_NO, ID, COUP_KIND_NAME, DISCOUNT, expires_period, INPUT_DATE, FLAG_PUBLISHABLE, DELETE_DATE, FLAG_DELETE, img from coupon_kind where flag_publishable=? and flag_delete='0'";
			pstmt = con.prepareStatement(selectCoupKind);
